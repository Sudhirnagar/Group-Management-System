...
const { logoutController } = require('../Controllers/authC');
const { authenticateToken, validateUserTypeS } = require('../Middlewares/jwtTokenVerifer');
const { fetchProfile, loadHomePage, joinCoursePage, joinCourseHandler } = require('../Controllers/Student/homeC');
...
// ✅ Join Course routes
router.route('/JoinCourse').get(joinCoursePage);     // render JoinCourse.hbs
router.route('/JoinCourse').post(joinCourseHandler); // handle form submission
